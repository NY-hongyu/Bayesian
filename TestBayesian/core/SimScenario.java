...
import routing.AffIndex;
import routing.BayesianRouter;
...
//		}
	...
	private void importAffRouter() throws Exception{
		MessageRouter mRouter;
		BayesianRouter bRouter;
		FileInputStream fi = null;
		ObjectInputStream ois;
		fi = new FileInputStream("C:/TheOne/Bayesian/testAffRouter.HashMap");
		...
		while (count<1) {
			...
			for (int i=0,n = hosts.size(); i<n; i++) {
				...
				bRouter = (BayesianRouter) mRouter;
				HashMap<Integer,ArrayList<AffIndex>> affRouter = new HashMap<Integer,ArrayList<AffIndex>>();
				try {
					Object obj = ois.readObject();
					if (obj!= null) { //if null,the end of the file
						//boolean res = ois.readObject() instanceof HashMap;
						//System.out.println(res);
						affRouter = (HashMap) obj;
						if(affRouter==null){
							System.out.println("null!!!!");
						}
						bRouter.mergeAffRouter(affRouter);
					} else {
						break;
					}
	...
	private void printAffRouter(){
		for (int i=0,n = hosts.size(); i<n; i++) {
			DTNHost host = hosts.get(i);
			MessageRouter mRouter = host.getRouter();
			BayesianRouter bRouter = (BayesianRouter) mRouter;
			Iterator ite = bRouter.getRouterMap().entrySet().iterator();
			while (ite.hasNext()){
				Map.Entry entry = (Map.Entry) ite.next();
				ArrayList affList = (ArrayList)entry.getValue();
				Iterator it = affList.iterator();
				while (it.hasNext()){
					AffIndex ai = (AffIndex) it.next();
					System.out.println("The router is   "+entry.getKey()+": "+ai.toString());
				}
			}
		}
	}
